...
from flask import redirect, url_for #삭제 후 '/'요청령로로 redirect

UPLOAD_FOLDER = './uploads/'

app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 1024 * 1024 * 5 #업로드 폴더 용량 제한
app.config['SECRET_KEY'] = 'secret!' #CSRF 보호 정책 설정을 하려면 필요
...
def download(filename):
    return send_file(UPLOAD_FOLDER + filename, 
                        as_attachment=True) #브라우저에서 파일이 열리지 않고 다운로드만 가능
