...
def info(filename):
    ...
    mtime = os.path.getmtime(UPLOAD_FOLDER + filename) #파일의 수정 시간
    atime = os.path.getatime(UPLOAD_FOLDER + filename) #파일의 접근 시간
    size = os.path.getsize(UPLOAD_FOLDER + filename) #파일의 크기(byte단위)
    if size >= 1024 * 1024:
        size = size / (1024 * 1024)
