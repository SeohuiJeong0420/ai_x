...
def on_send_mail(sender, **kwargs):
    ...
    if kwargs['created']: #True : 회원가입, False : 각종 회원정보 수정
        ...
        if not user.email: #회원가입시 메일 입력 안 함
            ...
            return
        #메일 보내기
        subject = f"{user.username}님 XX에 가입해주셔서 감사합니다"
        body = f"안녕하세요 {user.username}님\n\n 가입인사를 받으시기 바랍니다.\n\n 감사합니다.(메일내용)"
        bodyHtml = f"<h1>안녕하세요 {user.username}님</h1><p>가입인사를 받으시기 바랍니다.</p><p>감사합니다.(메일내용)</p>"
        # settings.py에 EMAIL_BACKEND 설정
        send_mail(
            subject = subject,
            message = body,
            from_email="b41628289@gmail.com",
            recipient_list=[user.email],
            html_message=bodyHtml,
            fail_silently=False, #메일 전송이 안 되었을 경우, 아무 일도 하지 않음
        )
