...
def update_now_report(wb, analysis, summary):
    #'prev_report'시트 삭제, 'now_repot'시트 복사(prev_report), 분석글now_report 업데이트
    #'prev_report'시트 삭제
    sheet_names = [s.name for s in wb.sheets]
    if 'prev_report' in sheet_names:
        ...
        print('prev_report 시트 존재하지 않아 삭제 불가')
    #'now_repot'시트 복사(prev_report)
    if 'now_report' in sheet_names:
        now_sheet = wb.sheets['now_report']
        prev_sheet = now_sheet.copy(after=now_sheet)
        prev_sheet.name = 'prev_report'
        print('pre_report시트 복사 완료')
    else:
    ...
    #now_sheet를 pdf 파일로 생성(genai_rpa_2507011655.pdf)
