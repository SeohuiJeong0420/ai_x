...
SELECT * FROM DEPTL; -- DEPT테이블의 모든 열, 모든행
SELECT * FROM SALGRADE;

-- 2. 특정열만 출력
DESC EMP;
   -- EMP테이블 구조를 나타내는 ORACLE명령
...
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL!=3000;
  ...
  -- EX1) 사원이름(NAME)이 'A', 'B', 'C'로 시작하는 사원의 모든 필드
  -- 'A' < 'AA' < 'AAA' < 'AAAA' < 'B' < 'C' <  'D'
...
-- 5. 산술연산자(SELECT절, 조건절, ORDER BY절)
  ...
  --모든사원의 사번, 이름, 상사사번 출력(상사가 없으면 'CEO'로 출력)
...
SELECT * FROM EMP ORDER BY SAL DESC, HIREDATE; --SAL기준 내림차순 정렬(SAL이 같으면 HIREDATE순으로 내림차순 정렬)

--연습문제 전 형변환 함수 : TO_CHAR, TO_DATE
-- ※날짜형(HIREDATE)을 문자형으로 변환: TO_CHAR(날짜형데이터, 패턴)
          --YYYY, YY, RR:년도/ MM 월 / DD 일 / DY 월 / DAY 월요일
          -- HH24, HH12, HH: 시 / AM 이나 PM / MI 분 / SS초
    SELECT ENAME, TO_CHAR(HIREDATE, 'YY/MM/DD HH12:MI:SS') FROM EMP;
    -- 82년전에 입사한 사원의 데이터
...
--3.	연봉(SAL*12+COMM)이 24000이상인 사번, 이름, 급여 출력 (급여순정렬)
 SELECT EMPNO, ENAME, SAL FROM EMP  WHERE (SAL*12+COMM) >= 24000 ORDER BY SAL;
