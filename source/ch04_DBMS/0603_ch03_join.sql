-- [III] join : 2개 이상의 테ㅣ블을 연결하여 데이터를 검식하는 방법
...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC
    ...
    SELECT EMPNO, ENAME, JOB, MGR, DEPTNO, DNAME, LOC
    FROM EMP E, DEPT D
    ...
    ORDER BY SAL;
    -- EX5. JOB이 'SALESMAN'이거나 'MANAGER'인 사원의 이름, 급여,상여, 연봉(SAL+COMM)*12, 부서명, 근무지(연봉이 큰 순)출력
    SELECT ENAME, SAL, COMM, (SAL+VNL(COMM,0))*12 
    FROM EMP E, DEPT D
...
SELECT * FROM EMP, SALGRADE
    ...
    SELECT EMPNO, ENAME, JOB, MGR, SAL, GRADE||'등급' GRADER
        FROM EMP, SALGRADE
    ...
    -- 탄탄2. -	이름, 급여, 입사일, 급여등급
    SELECT ENAME, SAL, HIREDATE, GRADE FROM EMP E, DEPT D, SALGRADE WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL;
...
SELECT EMPNO, ENAME, SAL, DNAME FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND SAL>=2000
ORDER BY SAL DESC;
...
SELECT W.ENAME, W.SAL, W.DEPTNO, LOC, NVL(M.ENAME, '없음') 직속상사
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR = M.EMPNO(+) AND W.DEPTNO=D.DEPTNO;
--6. 이름, 급여, 등급, 부서명, 직속상사명. 급여가 2000이상인 사람
SELECT W.ENAME, W.SAL, GRADE, DNAME, M.ENAME
    FROM EMP W, EMP M, DEPT D, SALGRADE
    WHERE W.MGR = M.EMPNO AND W.SAL>=2000 AND W.DEPTNO=D.DEPTNO AND W.SAL BETWEEN LOSAL AND HISAL;
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM,0))*12 연봉
    FROM EMP W, EMP M, DEPT D, SALGRADE
    WHERE W.MGR = M.EMPNO AND W.DEPTNO=D.DEPTNO AND W.SAL BETWEEN LOSAL AND HISAL;
...
SELECT W.ENAME, M.ENAME, MR.ENAME
    FROM EMP W, EMP M, EMP MR
    WHERE W.MGR = M.EMPNO AND M.MGR = MR.EMPNO;
